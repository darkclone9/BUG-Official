...
import { PointsMultiplier, PointsCategory } from '@/types/types';
import {
  createPointsMultiplier,
  getAllPointsMultipliers,
  updatePointsMultiplier,
  deletePointsMultiplier
} from '@/lib/database';
...
export default function PointsMultiplierManagement() {
  const { user } = useAuth();
  const [multipliers, setMultipliers] = useState<PointsMultiplier[]>([]);
  const [loading, setLoading] = useState(true);
  const [showCreateDialog, setShowCreateDialog] = useState(false);
  const [editingMultiplier, setEditingMultiplier] = useState<PointsMultiplier | null>(null);
  const [formData, setFormData] = useState({
    ...
    endDate: '',
    applicableCategories: ['event_attendance'] as PointsCategory[],
    isActive: true,
  });
  ...
  const loadMultipliers = async () => {
    try {
      setLoading(true);
      const data = await getAllPointsMultipliers();
      setMultipliers(data);
    } catch (error) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      toast.success('Points multiplier created successfully!');
  ...
  const handleUpdate = async () => {
    ...
    try {
      await updatePointsMultiplier(editingMultiplier.id, {
        name: formData.name,
        description: formData.description,
        multiplier: formData.multiplier,
        startDate: new Date(formData.startDate),
  ...
  const handleToggleActive = async (multiplier: PointsMultiplier) => {
    try {
      await updatePointsMultiplier(multiplier.id, {
  ...
  const handleDelete = async (multiplierId: string) => {
    if (!confirm('Are you sure you want to deactivate this multiplier?')) return;

    try {
      await deletePointsMultiplier(multiplierId);
      toast.success('Multiplier deactivated');
      loadMultipliers();
    } catch (error) {
      console.error('Error deleting multiplier:', error);
      toast.error('Failed to deactivate multiplier');
    }
  };

  const handleEdit = (multiplier: PointsMultiplier) => {
  ...
  const toggleCategory = (category: PointsCategory) => {
    setFormData(prev => ({
      ...prev,
  ...
  const isActive = (multiplier: PointsMultiplier) => {
    const now = new Date();
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Create campaigns to multiply points earned during specific periods
          </p>
        </div>
        <Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>
                {editingMultiplier ? 'Edit' : 'Create'} Points Multiplier Campaign
              </DialogTitle>
              <DialogDescription>
                Set up a campaign to multiply points earned during a specific period
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Points will be multiplied by this value (e.g., 2.0 = double points)
                </p>
              </div>
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-2">
                  Select which point earning activities will receive the multiplier
                </p>
              </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {multiplier.applicableCategories.map((category) => (
                      <Badge key={category} variant="secondary">
                        {POINTS_CATEGORIES.find(c => c.value === category)?.label || category}
                      </Badge>
                    ))}
                  </div>
                </div>
